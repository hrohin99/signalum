...
- **Competitive Dimensions Scoring:** Per-item importance tiers (Critical/High/Medium/Low) stored in the `importance` field of the JSONB `items` array on `competitive_dimensions`. Weighted scoring formula: Σ(item_score × weight) / Σ(weights), with tier weights critical=3, high=2, medium=1, low=0.5 and status scores yes=100, partial=50, unknown=25, no/na=0. Shared utility in `client/src/lib/dimensionScoring.ts`. `DimensionEditor.tsx` shows importance selects per item and supports drag-to-reorder of dimension cards using `@dnd-kit/sortable` (updates `priority` as 1-indexed position + `display_order` on drag). `DimensionComparisonCard.tsx` shows a coloured importance badge (C/H/M/L) next to each item name. Priority display badge removed from cards (priority is now a silent sort key). API layer defaults missing `importance` to "high" without DB migration.
