export function buildProfileContext(workspace: any): string {
  ...
  if (p === 'vendor') {
    ctx += `Competitors: ${((v:any)=>Array.isArray(v)?v.filter(Boolean):typeof v==="string"?v.replace(/^\{|\}$/g,"").split(",").map((s:string)=>s.replace(/^"|"$/g,"").trim()).filter(Boolean):[])(workspace.competitors).join(', ')}\n`;
    ...
    ctx += `Competing against: ${((v:any)=>Array.isArray(v)?v.filter(Boolean):typeof v==="string"?v.replace(/^\{|\}$/g,"").split(",").map((s:string)=>s.replace(/^"|"$/g,"").trim()).filter(Boolean):[])(workspace.competitors).join(', ')}\n`;
    ...
    ctx += `Regulations monitored: ${((v:any)=>Array.isArray(v)?v.filter(Boolean):typeof v==="string"?v.replace(/^\{|\}$/g,"").split(",").map((s:string)=>s.replace(/^"|"$/g,"").trim()).filter(Boolean):[])(workspace.regulationsMonitored||workspace.regulations_monitored).join(', ')}\n`;
    ctx += `Regulatory bodies: ${((v:any)=>Array.isArray(v)?v.filter(Boolean):typeof v==="string"?v.replace(/^\{|\}$/g,"").split(",").map((s:string)=>s.replace(/^"|"$/g,"").trim()).filter(Boolean):[])(workspace.regulatoryBodies||workspace.regulatory_bodies).join(', ')}\n`;
  ...
  if (Array.isArray(trackingTypes) && trackingTypes.includes('regulations')) {
    ctx += `Regulations tracked: ${((v:any)=>Array.isArray(v)?v.filter(Boolean):typeof v==="string"?v.replace(/^\{|\}$/g,"").split(",").map((s:string)=>s.replace(/^"|"$/g,"").trim()).filter(Boolean):[])(workspace.regulationsMonitored||workspace.regulations_monitored).join(', ')}\n`;
  }
  if (Array.isArray(trackingTypes) && trackingTypes.includes('standards')) {
    ctx += `Standards certified: ${((v:any)=>Array.isArray(v)?v.filter(Boolean):typeof v==="string"?v.replace(/^\{|\}$/g,"").split(",").map((s:string)=>s.replace(/^"|"$/g,"").trim()).filter(Boolean):[])(workspace.standardsCertified||workspace.standards_certified).join(', ')}\n`;
  }
  if (earlyWarning) {
