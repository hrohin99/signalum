  ...
  const FormRow = ({ onSubmit, onCancel, saving }: { onSubmit: () => void; onCancel: () => void; saving: boolean }) => (
    <div style={{ padding: '14px 18px', background: 'var(--color-background-secondary)', borderTop: '0.5px solid var(--color-border-tertiary)', display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
        <button data-testid="button-cancel-product" onClick={onCancel} style={{ fontSize: 12, padding: '4px 14px', border: '0.5px solid var(--color-border-tertiary)', borderRadius: 6, background: 'transparent', color: 'var(--color-text-secondary)', cursor: 'pointer' }}>Cancel</button>
    ...
    <div data-testid="card-products-solutions" style={{ background: '#ffffff', border: '1px solid #e8e8e8', borderRadius: 12, overflow: 'hidden', marginBottom: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 18px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span data-testid="text-products-count" style={{ fontSize: 11, padding: '2px 7px', borderRadius: 20, background: 'var(--color-background-secondary)', color: 'var(--color-text-secondary)', border: '0.5px solid var(--color-border-tertiary)' }}>{products.length}</span>
        </div>
        {canEdit && (
          ...
          return (
            <div key={p.id}>
              {isEditing ? (
                ...
                <div data-testid={`card-product-${p.id}`} style={{ padding: '13px 18px', borderBottom: '0.5px solid var(--color-border-tertiary)', display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span data-testid={`text-product-name-${p.id}`} style={{ fontSize: 13, fontWeight: 500, color: 'var(--color-text-primary)' }}>{p.product_name}</span>
                  ...
                  {p.description && <div data-testid={`text-product-description-${p.id}`} style={{ fontSize: 13, color: 'var(--color-text-secondary)', lineHeight: 1.6 }}>{p.description}</div>}
                  {tagList.length > 0 && (
